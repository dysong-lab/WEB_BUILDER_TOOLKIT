...
일반적인 추측 금지/검증 원칙은 루트 `CLAUDE.md`를 따릅니다. 아래는 Figma 변환 작업에서 **특히 유의할 고유 규칙**입니다.

### MCP 데이터가 유일한 원본

- `get_screenshot` → 유일한 시각 원본
- `get_design_context` → 정확한 구조와 수치 (Tailwind 클래스 → CSS 변환 시 값 그대로 사용)
- 시각적 차이가 보이면 CSS 값을 조정하지 말고 **MCP 데이터를 다시 확인**합니다
