...
| 조건 | SKILL |
|------|-------|
| 세션 학습 내용을 CLAUDE.md에 반영 (세션 종료 후 문서화) | `revise-claude-md` |
| CLAUDE.md 품질 점검 및 개선 (구조·최신성·실행가능성 진단) | `claude-md-improver` |
...
코드는 항상 읽기 쉽게 들여쓰기와 줄바꿈을 유지한다. HTML/CSS/JS 파일을 생성하거나 편집할 때 한 줄로 압축(minify)하지 않는다.

**금지 행위:** `<div class="a"><span>text</span></div>` 식으로 여러 태그를 한 줄에 몰아쓰기, CSS 규칙을 `;`으로 이어붙여 한 줄로 쓰기, JS 구문을 `,`·`;`으로 이어붙이기
**올바른 패턴:** 각 태그·규칙·구문을 별도 줄에 작성하고, 중첩 깊이에 맞게 2스페이스(또는 4스페이스) 들여쓰기
