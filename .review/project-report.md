...
03-28 검토에서는 "모든 register.js가 이 순서를 따른다"고 보고했으나, 03-31 전수 재검토에서 **차트 4종(BarChart, GaugeChart, LineChart, PieChart)의 Step 3(customEvents + bindEvents) 누락**을 발견하여 수정했습니다(`a3d4cc7`). 현재는 8개 컴포넌트 전부가 3단계를 완전히 따릅니다.
...
### 4-1. ✅ 코드-문서 정합성: 높음 (03-31 수정 후)

03-31 전수 재검토에서 발견한 이슈를 수정한 후, 8개 컴포넌트 + 4개 예제 프로젝트 전체가 규격을 준수합니다.

**03-31에 발견 → 수정한 이슈:**

| 이슈 | 대상 | 커밋 |
|------|------|------|
| 차트 4종 Step 3 (customEvents + bindEvents) 누락 | Components/ | `a3d4cc7` |
| 차트 4종 beforeDestroy Step 3 역순 (removeCustomEvents) 누락 | Components/ | `a3d4cc7` |
| 차트 4종 CSS/preview `#chart-container` → 컴포넌트별 고유 ID | Components/ | `a3d4cc7` |
| Sidebar customEvents: template 선택자 → 항목 선택자(menuid) | Components/ | `a3d4cc7` |
| Dashboard_Corporate EventBrowser: container 선택자 → item 선택자 | Examples/ | `6334430` |
| Dashboard_Corporate Sidebar: container 선택자 → menuid 선택자 | Examples/ | `6334430` |
| TabulatorMixin destroy()에서 `ns.init = null` 누락 | Mixins/ | `1d09c61` |
| HeatmapJsMixin `getInstance()` 미제공 (패턴 불일치) | Mixins/ | `1d09c61` |
...
SimpleDashboard의 5개 커스텀 컴포넌트, Dashboard_DarkTech/Minimal의 10개 컴포넌트도 동일한 패턴을 정확히 따릅니다.
...
#### ~~(A-2) 03-31 전수 재검토에서 발견된 코드/문서 불일치~~ → 수정 완료

차트 컴포넌트 Step 3 누락, 이벤트 선택자 오류, Mixin destroy/getInstance 불일치, 문서 경로 오류 등 8건을 발견하여 모두 수정함. 상세 목록은 4-1 섹션 참조.

추가로 수정된 문서:
- `README.md` 프로젝트 구조 트리 실제 디렉토리와 동기화 (`b004db0`)
- `RNBT_architecture/README.md` 참조 경로 텍스트 수정, 중복 구분선 제거 (`a3d4cc7`)
- `REVIEW_KEY_SHARING_DESIGN.md` 해결된 ack 이슈에 해결 표시, 마크다운 서식 정리 (`a3d4cc7`)
- `Figma_Conversion/README.md` 존재하지 않는 sample_test 제거 (`8d6667c`)
- 1-figma 스킬 참조 문서 최적화 — 순수 퍼블리싱에 불필요한 JS 규칙 참조 제거 (`1418441`)
- `FIGMA_MCP_GUIDE.md` "4가지 도구" → "MCP 도구" 수정 (`1418441`)
- `FIGMA_IMPLEMENTATION_GUIDE.md` 스크린샷 경로 예시 수정 (`1418441`)
- `create-symbol-state-component` 스킬 선택자 하드코딩 제거, 예제 링크 추가 (`f7f1799`)
- `create-project` 스킬 출력 구조에서 Mixins/ 폴더 제거 (`b1f09fd`)
...
| 코드 정합성 | ◎ 우수 | 03-31 전수 재검토에서 8건 발견 → 모두 수정 완료. 현재 35개 컴포넌트 전부 패턴 준수 |
| 문서 정합성 | ◎ 우수 | 03-28 수정 1건 + 03-31 문서/스킬 수정 9건 → 모두 수정 완료 |
...
03-31 전수 재검토에서 코드/문서 불일치 8건 + 문서/스킬 수정 9건을 발견하여 모두 수정했습니다. **03-28 검토에서 놓친 이슈(차트 Step 3 누락, 이벤트 선택자 오류, Mixin destroy 불완전)가 있었으며, 전수 검증의 중요성을 재확인했습니다.**
