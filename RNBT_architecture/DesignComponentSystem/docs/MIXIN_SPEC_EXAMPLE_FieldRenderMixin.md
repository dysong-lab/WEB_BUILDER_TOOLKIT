...
|------|------|
| **목적** | 데이터를 보여준다 |
| **수단** | 이미 존재하는 DOM 요소에 값을 채운다 |
| **기능** | 플랫 객체의 각 필드를 CSS 선택자로 지정된 요소에 매핑하여 렌더링한다 |
...
| **수단의 차이** | ListRenderMixin은 template을 복제하여 배열 데이터로 DOM을 생성함. FieldRenderMixin은 이미 존재하는 DOM 요소에 값을 채움. 데이터 형태도 다름 (배열 vs 플랫 객체). |
