...
| **출력** | `Components/` 하위 카테고리 디렉토리 + 각 카테고리의 CLAUDE.md (MD3 정의 + 역할) |
...
| **세분화 단위** | MD3 하위 범주 (TopAppBar, IconButtons 등) | 기능 변형 (01_status, 02_camera, 03_popup) |
...
| **활동** | (1) MD3 명세에서 범주에 하위 범주가 있는지 확인한다. 하위 범주가 있으면 각 하위 범주별로, 없으면 범주 자체에서 기능을 분석한다. (2) 각 항목의 **행동/상태/상호작용**을 도출한다. (3) 도출된 기능을 "기능의 정의" 프레임으로 검증한다. (4) **기능이 같으면 하나의 컴포넌트**(디자인 변형으로 처리), **기능이 다르면 별도 컴포넌트**로 분리한다. |
...
1. MD3 명세(m3.material.io)에서 해당 컴포넌트 페이지를 확인한다
   - WebFetch 실패 시 (SPA 빈 페이지, 403 등):
     대안 A — 범주 CLAUDE.md + 학습 데이터 기반 분석
     대안 B — WebSearch로 MD3 관련 문서 검색
   - 어떤 경로든 분석 결과를 사용자에게 제시하고 승인 후 진행한다
