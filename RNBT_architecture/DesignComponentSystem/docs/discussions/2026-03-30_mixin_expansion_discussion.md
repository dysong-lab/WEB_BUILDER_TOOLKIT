...
| 기존 대안 | FieldRenderMixin — data-* 속성 설정 → CSS가 스타일링 |

**기존으로 되는 것:**
- `data-status="normal"` → CSS에서 색상/배경 변경 → **가능**
- CSS `content` 속성으로 간단한 아이콘 문자 교체 → **가능**

**기존으로 안 되는 것:**
- `<img>` 태그의 `src` 교체 (상태별 다른 이미지)
- SVG 요소의 `fill` 색상 직접 변경
- 복수 속성 동시 변경 (아이콘 + 색상 + 라벨 + 배지)

FieldRender의 수단: "textContent에 필드값을 넣는다" 또는 "data-* 속성을 설정한다"
아이콘 교체의 수단: "요소의 src/fill/className을 상태값에 따라 변경한다"

→ **"어떻게 반영하는가?"의 답이 다르다** (textContent/data-* vs src/fill/className)

**판정: 새 Mixin 필요 → IconStateMixin**
...
{
  cssSelectors: {
    pump1: '.equipment[data-id="pump1"] .icon',
    pump2: '.equipment[data-id="pump2"] .icon',
    valve1: '.equipment[data-id="valve1"] .icon'
  },
  stateMap: {
    running: { src: '/icons/pump-on.svg', fill: '#22c55e', className: 'status--running' },
    stopped: { src: '/icons/pump-off.svg', fill: '#94a3b8', className: 'status--stopped' },
    fault:   { src: '/icons/pump-fault.svg', fill: '#ef4444', className: 'status--fault' }
  }
}
```

핵심 메서드: `renderData`, `getState(key)`
...
→ 달성률 바는 `style.width`를 조작해야 한다. FieldRender는 textContent와 data-* 만 다룬다. `style` 조작은 **다른 수단**이다.
→ 추이 방향은 데이터 변환이므로 Mixin 바깥(서버 또는 조립 코드)에서 계산하여 `trend: "up"` 같은 필드로 전달하면, data-trend 속성 + CSS로 화살표 표시 가능.

**판정 분기:**

- 달성률 바를 포기하고 CSS `background` gradient로 대체 가능하다면 → FieldRender로 충분
- `style` 조작이 필수라면 → FieldRender를 확장하거나 새 Mixin

→ 하지만 "style 조작이 필요한 경우"는 KPI 카드에만 국한되지 않는다. 프로그레스 바, 게이지 바, 용량 표시 등 **값을 CSS 속성(width, height, transform)으로 시각화하는 패턴**이 범용적이다.

**결론: 보류 — FieldRender에 style 바인딩 기능을 추가할지, 별도 Mixin(StyleBindMixin?)을 만들지는 추가 논의 필요**
...
**Mixin:** StatefulListRender(경보 목록 + 상태 전환) + FieldRender(카운트 요약) + IconState(등급 아이콘)

```
topic: alarmList → [{ alarmId, severity, message, source, time, status }, ...]
...
**Mixin:** TreeRender(계층 구조) + IconState(노드별 상태 아이콘)
...
**사용 도메인:** 전 도메인 공통
**Mixin:** IconState(설비별 상태 아이콘) + ShadowPopup(상세 팝업) + FieldRender(팝업 내부)

```
topic: equipmentStatus → { pump1: "running", pump2: "stopped", valve1: "fault", ... }
```
...
**사용 도메인:** DCIM, IT 인프라
**Mixin:** StatefulListRender(슬롯별 장비) + IconState(장비 아이콘 + 상태) + ECharts(실시간 지표 차트)
