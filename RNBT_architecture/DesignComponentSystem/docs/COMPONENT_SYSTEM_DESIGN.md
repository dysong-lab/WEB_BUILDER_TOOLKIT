...
1. **cssSelectors / datasetSelectors** — KEY는 Mixin이 정의하고, VALUE는 HTML에서 온다.
2. **데이터 변환은 Mixin 바깥** — Mixin은 이미 selector KEY에 맞춰진 데이터만 받는다. 변환은 구독 연결 단계에서 처리한다.
...
| 선택자 | KEY | VALUE |
|--------|-----|-------|
| cssSelectors | Mixin이 정의한 인터페이스 이름 | CSS 선택자 (HTML에서 온다) |
| datasetSelectors | Mixin이 정의한 인터페이스 이름 | data-* 속성명 (HTML에서 온다) |

cssSelectors에는 데이터 바인딩용 선택자와 이벤트 바인딩용 선택자가 모두 포함될 수 있다. dataFormat에 해당 키가 없으면 데이터 바인딩은 건너뛴다.

마크업 시점에 `data-*` 속성이 선언되면, 그 요소는 datasetSelectors의 대상이 된다. datasetSelectors의 VALUE는 속성명만 기술한다 (Mixin이 내부적으로 `[data-속성명]` 선택자를 조립한다).
