...
5.  데이터 포맷 = dataFormat 함수 (selectors KEY로 조회하여 데이터 매핑)
...
2. **dataFormat** — dataFormat의 키로 cssSelectors/datasetSelectors를 조회하여, 해당 위치에 데이터를 매핑한다.
    ...
    ns.renderData = function({ response }) {
        ...
        formatted.items.forEach(item => {
            ...
            Object.entries(datasetSelectors).forEach(([key, attr]) => {
                ...
                if (el && item[key] != null) el.dataset[attr] = item[key];
            });
...
dataFormat의 KEY로 cssSelectors/datasetSelectors를 **조회**한다:

```
dataFormat의 KEY → cssSelectors[KEY]     → CSS 선택자 → querySelector → textContent
dataFormat의 KEY → datasetSelectors[KEY] → 속성명     → querySelector('[data-속성명]') → dataset
```

```javascript
dataFormat: (data) => ({
    name:        data.hostname,         // → cssSelectors['name'] 조회 → textContent
    status:      data.status,           // → datasetSelectors['status'] 조회 → dataset
    statusLabel: data.statusLabel,      // → cssSelectors['statusLabel'] 조회 → textContent
    version:     data.version
})
```
...
- dataFormat의 KEY로 cssSelectors/datasetSelectors를 조회하여 대상 요소를 찾는다
