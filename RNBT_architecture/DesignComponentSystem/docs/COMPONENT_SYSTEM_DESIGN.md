...
│  └── register.js (조립 코드)                                │
│       │                                                     │
│       ├── applyMixinA(this, { cssSelectors, datasetSelectors })│
...
3.  각 Mixin이 받는 옵션이 그 Mixin의 인터페이스
    - 2D DOM Mixin: cssSelectors/datasetSelectors (선택자 계약)
    - 그 외: Mixin별 문서 참고
...
각 KEY의 역할은 Mixin별 문서를 참고한다. 다른 Mixin(ChartRender, TableRender, HeatmapRender, MeshState, CameraFocus)은 각자의 성격에 맞는 옵션을 받으며, Mixin별 문서를 참고한다.

> **두 객체의 차이:** cssSelectors의 VALUE는 DOM 요소를 찾는 선택자이고, datasetSelectors의 VALUE는 DOM 속성명이 된다. 개발자는 Mixin이 사용하지 않는 KEY도 추가할 수 있으며, cssSelectors는 JS에서, datasetSelectors는 CSS에서도 활용된다.
...
예를 들어 ListRenderMixin의 renderData는 datasetSelectors의 모든 항목을 DOM의 data-* 속성으로 반영한다. CSS에서 `[data-severity="critical"]` 같은 스타일링이 필요하다면, 개발자가 datasetSelectors에 해당 항목을 정의해야 한다. 이처럼 Mixin이 객체를 어떤 매커니즘으로 소비하는지 알아야 개발자가 자신에게 필요한 항목을 판단하여 등록할 수 있다.
...
| applyChartRenderMixin | `this.chartRender` |
| applyTableRenderMixin | `this.tableRender` |
| applyHeatmapRenderMixin | `this.heatmapRender` |
