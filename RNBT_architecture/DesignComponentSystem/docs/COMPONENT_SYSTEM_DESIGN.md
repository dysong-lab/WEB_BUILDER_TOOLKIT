...
특수한 행위는 사전에 준비할 수 없다 — 프로젝트 요구가 있어야 식별된다. 프로젝트에서 대응하되, 여러 프로젝트에서 반복되면 일반적 행위로 승격하여 Mixin으로 추출한다.
...
3.  선택자 = 컴포넌트↔Mixin 계약 인터페이스
    ...
    - cssSelectors: KEY → CSS 선택자(VALUE)로 HTML 요소를 찾아 참조
    - datasetSelectors: KEY → data-* 속성명(VALUE)으로 HTML 요소를 찾아 참조
4.  구독 = Mixin 바깥 (topic은 프로젝트 레벨)
    - subscriptions 객체로 관리, 함수 참조 사용
5.  데이터 변환 = Mixin 바깥 (구독 연결 단계에서 selector KEY에 맞춰 변환)
...
this.dataFormats = {
    systemInfo: (data) => ({
        name:        data.hostname,
        status:      data.status,           // → datasetSelectors['status'] → dataset
        statusLabel: data.statusLabel,      // → cssSelectors['statusLabel'] → textContent
...
| 데이터 변환 함수 정의 (dataFormats) | 상태 관리 (_state 등) |
| 구독 연결 (subscriptions) | fetch 호출 |
| 이벤트 매핑 (customEvents) | Mixin 메서드 재정의 |
...
1. **cssSelectors / datasetSelectors를 주입한다** — Mixin이 KEY를 정의하고, 컴포넌트가 HTML에 맞춰 VALUE를 제공한다.
2. **Mixin은 이미 변환된 데이터를 받는다** — 데이터의 KEY가 selector KEY와 일치해야 한다. 변환은 Mixin 바깥(페이지)에서 처리한다.
3. **Mixin의 기본 역할은 여기까지다.** 추가 기능(updateItemState 등)은 Mixin별 문서를 참고한다.

Mixin에 주입되는 것은 **cssSelectors와 datasetSelectors 두 객체**뿐이다. 데이터 변환 로직은 Mixin의 관심사가 아니다.

### 선택자 구분

Mixin은 두 종류의 선택자를 받는다.

| 선택자 | KEY | VALUE | Mixin 내부 동작 |
|--------|-----|-------|----------------|
| cssSelectors | 인터페이스 이름 | CSS 선택자 | `querySelector(VALUE)` → `textContent` |
| datasetSelectors | 인터페이스 이름 | data-* 속성명 | `querySelector('[data-' + VALUE + ']')` → `dataset` |

cssSelectors에 데이터 바인딩 KEY가 있어도 데이터에 해당 KEY가 없으면 건너뛴다 (이벤트 전용 선택자).
...
| renderData 입력 | 플랫 객체 `{ key: value }` | 배열 `[{ key: value }, ...]` |
...
cssSelectors와 datasetSelectors는 세 가지를 연결한다:

```
1. Mixin → KEY를 정의한다
   "나는 이 KEY들로 동작한다. 이 KEY에 해당하는 HTML 위치를 알려달라."

2. HTML → VALUE를 제공한다
   "이 KEY에 해당하는 요소는 이 선택자/속성명으로 찾을 수 있다."

3. 데이터 → KEY로 조회된다
   "이 KEY에 해당하는 데이터를 이 위치에 렌더링한다."
```

KEY 하나가 "어디에(VALUE)"와 "무엇을(데이터)"을 동시에 연결하는 열쇠다.

### KEY의 두 종류

| 종류 | 정의 주체 | 예시 |
|------|----------|------|
| 구조적 KEY | Mixin이 고정적으로 정의 | container, item, template, itemKey |
| 데이터 필드 KEY | 컴포넌트가 HTML에 맞춰 정의 | name, status, severity, ack |

구조적 KEY는 Mixin 내부에서 특별한 로직에 사용된다 (컨테이너 찾기, 항목 식별 등). 데이터 필드 KEY는 Mixin이 순회하면서 렌더링하는 대상이다.
...
cssSelectors에 데이터 바인딩 KEY가 있어도 데이터에 해당 KEY가 없으면 건너뛴다 (이벤트 전용 선택자). datasetSelectors의 대상은 마크업 시점에 `data-*` 속성으로 이미 선언되어 있다.

### renderData가 KEY를 사용하는 흐름

```
데이터 도착: { name: 'server-01', status: 'RUNNING', statusLabel: '정상' }
             ↓ KEY별 조회

KEY 'name':
  cssSelectors['name'] = '.system-info__name'
  → querySelector('.system-info__name')
  → el.textContent = 'server-01'

KEY 'status':
  datasetSelectors['status'] = 'status'
  → querySelector('[data-status]')
  → el.dataset.status = 'RUNNING'

KEY 'statusLabel':
  cssSelectors['statusLabel'] = '.system-info__status'
  → querySelector('.system-info__status')
  → el.textContent = '정상'
```
...
register.js (조립 코드):
  ...
  // → Mixin은 이미 selector KEY에 맞춰진 데이터를 받아 렌더링
...
subscriptions는 **함수 참조** 배열을 사용한다. Mixin 적용 이후에 선언해야 한다. 데이터 변환이 필요하면 구독 핸들러에서 변환 후 Mixin에 전달한다.

```javascript
// Mixin 적용 후 — 데이터 변환 + 구독 연결
this.dataFormats = {
    systemInfo: (data) => ({
        name: data.hostname,
        status: data.status
...
Mixin은 데이터 출처를 알지 못한다. Mixin은 **이미 selector KEY에 맞춰진 데이터**만 받는다.

API 응답과 selector KEY 사이의 간극은 **구독 연결 단계**에서 해결한다. Mixin 바깥에서 변환한 후, Mixin에 전달한다.

```javascript
// register.js — 구독 연결 단계에서 변환
this.dataFormats = {
    systemInfo: (data) => ({
        name:        data.hostname,         // API 키 → selector KEY
        status:      data.status,
...
배경에서 구분한 **특수한 행위**(콘텐츠에 의존하는 행위)는 Mixin이 아니라 프로젝트가 담당한다.
...
프로젝트 특화 행위(배경에서 구분한 **특수한 행위**)가 여러 프로젝트에서 반복되면, 그것은 일반적 행위로 승격된다. 도메인 Mixin으로 추출하며, 같은 원칙(네임스페이스 주입, 선택자 계약, 자체 destroy)을 따른다.
