...
| 요소 | 소유하는 것 | 소유하지 않는 것 |
|------|-------------|------------------|
| 컴포넌트 | HTML, CSS, 조립 코드 | Mixin 재정의 코드, 데이터 fetch |
...
| 데이터 변환 메서드 정의, 이벤트 매핑 (customEvents) | Mixin 메서드 재정의, 내부 상태 직접 조작 |
...
| CameraFocusMixin | 카메라를 특정 대상으로 이동 | `this.cameraFocus` |
