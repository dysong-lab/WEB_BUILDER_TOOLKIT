/**
 * gltf_container — 03_status_popup / page / before_load
 *
 * 3D 페이지 before_load 시점
 * - 이벤트 핸들러 등록 (Mesh 클릭 → 팝업 표시)
 * - 3D 레이캐스팅 설정
 */

const { onEventBusHandlers, initThreeRaycasting, withSelector, fetchData } = Wkit;
...
this.pageEventBusHandlers = {
    '@meshClicked': ({ event, targetInstance }) => {
        ...
        fetchData(this, 'meshDetail', { meshName })
            .then(data => targetInstance.showDetail(meshName, data))
            .catch(err => console.error(`[Page] meshDetail:`, err));
    }
};
