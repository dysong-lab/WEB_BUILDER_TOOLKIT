# thermohygrostat — Advanced/sensorDataHud

## 기능 정의

1. **상태 색상 표시** — equipmentStatus 토픽으로 수신한 데이터에 따라 'thermohygrostat' Mesh 색상 변경 (Standard 승계, `material.color` 채널)
2. **실시간 온습도 수치 라벨 (DOM 오버레이)** — 'thermohygrostat' mesh의 월드 좌표를 카메라 projection으로 화면 좌표로 변환하여, 이를 따라가는 absolute-position된 DOM 카드에 온도(℃) + 습도(%) 두 수치를 실시간 표시
   - 운영자가 3D 모델을 자유롭게 회전·줌해도 라벨이 thermohygrostat 위에 고정되어 따라온다
...
3. **임계값 초과 시 emissive 펄스 + 라벨 색상 변경** — 온도 또는 습도가 정상 범위(`tempMin~tempMax`, `humiMin~humiMax`)를 벗어나면:
   - 'thermohygrostat' mesh `material.emissive` 펄스 자동 활성 (BATT/alarmPulse 패턴 답습)
...
> 본 변형은 직전 사이클 `tempHumiTH2B/Advanced/sensorDataHud`의 100% 답습이다. mesh name(`tempHumiTH2B` → `thermohygrostat`)과 GLTF 모델 경로만 다를 뿐 그 외 모든 로직(시그니처·기본값·임계값·펄스 옵션·라이프사이클·beforeDestroy)은 완전 동일.

---

## thermohygrostat mesh 구조 결정

| 항목 | 값 |
|------|-----|
| GLTF 경로 | `models/thermohygrostat/01_default/thermohygrostat.gltf` |
| mesh 이름 | `thermohygrostat` (단일) |
| 결정 | **단일 mesh** — 개별 단위(1 GLTF = 1 Mesh) 패턴 적용 |

근거: 컴포넌트 루트 CLAUDE.md의 `유형 = 개별 (1 GLTF = 1 Mesh)`와 일치하며 Standard/Advanced/camera/popup 모두 단일 'thermohygrostat' 메시 기반으로 동작 중. 본 변형도 그 규약을 그대로 따른다.
...
```
mesh = appendElement.getObjectByName('thermohygrostat')
// 매 프레임:
...
> **자동 start 규약**: BATT/dataHud는 페이지 명시 start 규약이지만, 본 변형은 GasDetector/sensorHud · chargeFlowArrow · tempHumiTH2B/sensorDataHud처럼 **mount 직후 자동 start with 기본 데이터/임계값**으로 시각 관찰성을 우선한다 (Phase 1.5 항목 #7).
...
온습도 수치는 별도 토픽 없이 페이지가 직접 `instance.sensorDataHud.setData(...)`를 호출하는 외부 명령형 패턴 (BATT/dataHud, GasDetector/sensorHud, tempHumiTH2B/sensorDataHud와 동일). 향후 `tempHumidity` 같은 표준 토픽이 정의되면 페이지가 자체 어댑터에서 `sensorDataHud.setData`로 위임하거나, register에서 직접 구독하도록 확장 가능.

### 이벤트 (customEvents)

없음. 개별 단위(meshName='thermohygrostat' 확정)이므로 `@meshClicked` 같은 동적 식별 이벤트 불필요.
...
## tempHumiTH2B/sensorDataHud와의 시그니처 동일성 (Mixin 승격 핵심 근거)

본 변형은 `tempHumiTH2B/Advanced/sensorDataHud`의 **100% 답습**이다. 두 컴포넌트는 mesh name과 GLTF 모델 경로 외에 어떤 차이도 없다. 신규 Mixin 승격(`SensorDataHudMixin` 가칭) 시 호환성 깨짐 없이 두 컴포넌트를 한꺼번에 마이그레이션할 수 있는 결정적 근거.

| 항목 | tempHumiTH2B/sensorDataHud | thermohygrostat/sensorDataHud | 동일성 |
|------|----------------------------|--------------------------------|--------|
| Mixin 조합 | MeshStateMixin + FieldRenderMixin + 커스텀 | MeshStateMixin + FieldRenderMixin + 커스텀 | ✓ 100% |
| colorMap | `{normal:0x34d399, warning:0xfbbf24, error:0xf87171, offline:0x6b7280}` | 동일 | ✓ 100% |
| cssSelectors | `{temperature:'.th-temperature', humidity:'.th-humidity', stateLabel:'.th-state', state:'.th-state'}` | 동일 | ✓ 100% |
| datasetAttrs | `{state:'state'}` | 동일 | ✓ 100% |
| 커스텀 메서드 | `setData/setThresholds/setOffset/setPulseOptions/show/hide/start/stop/destroy` | 동일 | ✓ 100% |
| setData 입력 | `{temperature, humidity}` | 동일 | ✓ 100% |
| DEFAULT_SEED | `{temperature:25, humidity:50}` | 동일 | ✓ 100% |
| DEFAULT_THRESHOLDS | `{tempMin:18, tempMax:28, humiMin:30, humiMax:70}` | 동일 | ✓ 100% |
| DEFAULT_PULSE_OPTS | `{color:0xff3030, period:700, minIntensity:0, maxIntensity:1.5}` | 동일 | ✓ 100% |
| offset 기본값 | `{x:0, y:-16}` | 동일 | ✓ 100% |
| autoStart on mount | true | true | ✓ 100% |
| 구독 토픽 | `equipmentStatus → meshState.renderData` | 동일 | ✓ 100% |
| 라이프사이클 | self-null destroy + 적용 역순 정리 | 동일 | ✓ 100% |
| **mesh name** | `'tempHumiTH2B'` | `'thermohygrostat'` | ✗ — **유일한 차이** |
| **GLTF 경로** | `models/tempHumiTH2B/01_default/tempHumiTH2B.gltf` | `models/thermohygrostat/01_default/thermohygrostat.gltf` | ✗ — **유일한 차이** |

**옵션 차이**: 0개. 시그니처 동일성 100%. mesh name만 옵션화하면 즉시 Mixin으로 통합 가능.

---

## Mixin 승격 검토 시점 (강화)

### ADVANCED_QUEUE.md 정책 도달

ADVANCED_QUEUE.md "커스텀 메서드 vs 신규 Mixin 판단 규칙" — **"2번째 컴포넌트 등록 시점에 Mixin 승격 검토"** 정책의 결정적 시점이 **본 사이클(thermohygrostat/sensorDataHud)**이다.

| 카운트 | 컴포넌트 | 사이클 | 시그니처 |
|--------|---------|--------|----------|
| 1번째 | `tempHumiTH2B/Advanced/sensorDataHud` | 직전 사이클 (완료) | 베이스 |
| **2번째** | `thermohygrostat/Advanced/sensorDataHud` | **본 사이클 (완료)** — **승격 검토 시점 도달** | 100% 동일 |

### 본 사이클의 의사결정

- **신규 Mixin 생성 금지** (메인 루프 정책) — 본 사이클은 **메모만 남기고 마이그레이션은 별도 사이클에서 일괄 수행**
- 사용자가 별도 사이클(`create-mixin-spec` + `implement-mixin`)을 실행하면 **두 컴포넌트(tempHumiTH2B + thermohygrostat)를 한꺼번에 마이그레이션** 가능

### 신규 Mixin 후보 사양 (메모)

| 항목 | 권고 |
|------|------|
| **이름 (1안)** | `SensorDataHudMixin`(가칭) — 현재 시그니처 그대로 패키지: DOM 라벨 + 좌표 RAF + 임계값 비교 + emissive 펄스 자동 토글까지 통합 |
| **이름 (2안 — 단계적 분리)** | `Worldspace2DOverlayMixin`(가칭, 라벨/좌표만) + `MeshEmissivePulseMixin`(가칭, 펄스만) → 컴포넌트 register에서 두 Mixin 조합 + 임계값 비교 래퍼만 작성 |
| **공개 시그니처** | `setData/setThresholds/setOffset/setPulseOptions/show/hide/start/stop/destroy` — 현 두 컴포넌트와 100% 호환, 깨짐 없음 |
| **Mixin 옵션** | `meshName`(string, 필수), `cssSelectors`, `datasetAttrs`, `defaultSeed`, `defaultThresholds`, `defaultPulseOpts`, `defaultOffset`, `stateLabels`, `autoStart`(boolean, 기본 true) |
...
| 두 컴포넌트 register 변경 | `applyFieldRenderMixin + 커스텀 sensorDataHud` 약 350줄 → `applySensorDataHudMixin({ meshName, ... })` 약 5~20줄로 축소 |
| 향후 추가 컴포넌트 | #26 `MonnitTemperature_sensor/sensorDataHud`(예상)는 단채널(온도)이지만 같은 Mixin로 `cssSelectors` 옵션만 다르게 부여하면 즉시 적용 가능 |
...
<div id="th-hud-overlay" style="position:absolute; inset:0; pointer-events:none;">
    <div class="th-hud-card" data-mesh-name="thermohygrostat">
...
this.thInst = wemb.getDesignComponent('thermohygrostat');
...
- `models/thermohygrostat/01_default/thermohygrostat.gltf`의 단일 메시 이름은 `'thermohygrostat'`로 확정. sensorDataHud는 `getObjectByName('thermohygrostat')`로 추적/펄스 대상 mesh를 직접 조회한다.
...
| 5 | 커스텀 메서드 시그니처 일관성 | OK — `setData/setThresholds/setOffset/setPulseOptions/show/hide/start/stop/destroy` (tempHumiTH2B/sensorDataHud와 100% 동일) |
...
| 8 | manifest + thermohygrostat/CLAUDE.md 등록 | 본 사이클에서 처리 (ADVANCED_QUEUE는 메인 루프 담당) |

---

## 발견한 문제 / Mixin 승격 후보

- **Mixin 승격 검토 시점 도달 (결정적)**: `SensorDataHudMixin`(가칭) — **2번째 컴포넌트 적용 — tempHumiTH2B + thermohygrostat 시그니처 100% 동일**. ADVANCED_QUEUE.md "2번째 컴포넌트 등록 시점에 Mixin 승격 검토" 정책의 결정적 시점이 본 사이클이다. 본 사이클은 신규 Mixin 생성 금지이므로 메모만 남긴다. 사용자가 별도 사이클(`create-mixin-spec` + `implement-mixin`)을 실행하면 두 컴포넌트를 한꺼번에 마이그레이션할 수 있도록 위 "tempHumiTH2B/sensorDataHud와의 시그니처 동일성" + "Mixin 승격 검토 시점 (강화)" 섹션에 충분한 근거(시그니처 동일성·옵션 차이 0개·이름 후보 1안/2안)를 기록했다.
- **펄스 색상 차등 미적용**: 온도 알람 vs 습도 알람을 색상으로 구분할 수도 있었으나(예: 온도=빨강, 습도=파랑), 시각적 혼동 우려 + Mixin 승격 시 단순 시그니처 보존 우선으로 **빨강 1색 통일**. 라벨 수치로 어느 채널이 알람인지 식별 가능하므로 정보 손실 없음. 직전 사이클(tempHumiTH2B/sensorDataHud)과 동일 결정.
- **자동 start 규약 차이**: dataHud/alarmPulse는 페이지 명시 start 규약이지만 본 변형은 sensorHud/chargeFlowArrow/tempHumiTH2B/sensorDataHud처럼 **mount 직후 자동 start**. 향후 Mixin 승격 시 옵션화(`options.autoStart`) 권장.
- **`setData` 토픽 표준화 후보**: 현재 페이지가 외부 명령형으로 `setData` 호출하지만, 온습도 센서 데이터 토픽이 표준화되면 register에서 `tempHumidity` 토픽 직접 구독으로 단순화 가능. (tempHumiTH2B와 공통 — Mixin 승격 시 `subscribeTopic` 옵션 도입 권장)
