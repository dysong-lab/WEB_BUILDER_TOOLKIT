# Marker_OutdoorHydrant — Advanced/emergencyZoneRadius

## 기능 정의

1. **상태 색상 표시** — equipmentStatus 토픽으로 수신한 데이터에 따라 'MarkerOutdoorHydrant_A' Mesh의 단일 material(`Material #45`) 색상 변경 (Standard 승계, `material.color` 채널)
2. **보호 영역 반투명 구체 시각화** — Marker_OutdoorHydrant 주변에 반투명 구체(`SphereGeometry` + 반투명 `MeshBasicMaterial`)를 자체 생성하여 옥외 소화전의 보호/관할 영역(소화 호스 도달 거리)을 시각화 (GasDetector/sensorHud의 sphere 채널 답습 + Marker_Firealarm/emergencyZoneRadius #62 + Marker_IndoorHydrant/emergencyZoneRadius #63 답습)
...
3. **비상 시 발광 증가** — emergency 모드 활성 시 두 시각 채널이 함께 sine wave 변조 (LeakDetector/leakAlarmPulse 펄스 패턴 + #62/#63 답습)
   - **Marker_OutdoorHydrant mesh의 `material.emissive`** sine wave 변조 (emissive 광휘 펄스, emissive color로 비상 색상 발광)
...
> **#62 Marker_Firealarm + #63 Marker_IndoorHydrant/emergencyZoneRadius와 시그니처 100% 동일**. 모델 차이(meshName/material id)만 보정하고 알고리즘·옵션 기본값·라이프사이클은 완전히 답습.

---

## Mixin 승격 임계점 3개 도달 — 메인 루프 외부 수동 승격 강력 권장

- **#62 Marker_Firealarm + #63 Marker_IndoorHydrant + #64 Marker_OutdoorHydrant 동일 시그니처 100% 일치** — 세 컴포넌트가 동일 변형명(`emergencyZoneRadius`), 동일 도메인(emergency zone marker), 동일 API(`setEmergency/setRadius/setColor/setPulseOptions/show/hide/start/stop/destroy`), 동일 알고리즘(SphereGeometry 자체 생성 + emissive sine 펄스 + sphere opacity 동기 펄스), 동일 옵션 기본값(period 900ms, idleOpacity 0.18, maxOpacity 0.55, maxEmissive 1.5, color 0xff3030, radius factor 1.5)을 공유한다.
- ADVANCED_QUEUE 승격 4조건 중 **(1) 기법 동일성 + (2) API 호환성을 명백히 만족**한다. 이는 ADVANCED_QUEUE의 **2번째 컴포넌트 등록 시점에 Mixin 승격을 검토**한다는 2차 규칙을 이미 초과한 상태이다.
- **강력 후보**: `EmergencyZoneRadiusMixin` 단일 또는 (`RadiusSphereMixin` + `MeshEmissivePulseMixin`) 분리
- **분리 안의 장점**: `RadiusSphereMixin`은 GasDetector/sensorHud의 sphere 부분과 통합 가능 (재사용성 4개로 확대); `MeshEmissivePulseMixin`은 LeakDetector/leakAlarmPulse의 emissive 펄스 부분과 통합 가능
- **권장 시점**: 본 사이클(#64) 완료 후 **메인 루프 외부에서 사용자 수동 작업**으로 `create-mixin-spec` → `implement-mixin` 호출. 본 사이클은 메인 루프 신규 Mixin 금지 정책으로 커스텀 메서드 유지하되, 승격 임계점 3개 도달 사실을 명시적으로 기록한다.

---

## Marker_OutdoorHydrant mesh 구조 결정

| 항목 | 값 |
|------|-----|
| GLTF 경로 | `models/Marker_OutdoorHydrant/01_default/Marker_OutdoorHydrant.gltf` |
| mesh 이름 | **`MarkerOutdoorHydrant_A`** — 폴더명 `Marker_OutdoorHydrant`(`Marker` + `_` + `OutdoorHydrant`)과 **구분자 위치 + 접미 불일치**. `getObjectByName`은 이름을 그대로 비교하므로 반드시 `'MarkerOutdoorHydrant_A'`(언더스코어 제거 + A)로 조회 |
| 결정 | **단일 mesh** — 개별 단위(1 GLTF = 1 Mesh) 패턴 적용. material은 단일 객체 (`Material #45`) |

근거: 컴포넌트 루트 CLAUDE.md의 `유형 = 개별 (1 GLTF = 1 Mesh: MarkerOutdoorHydrant_A)`와 일치. Standard도 `'MarkerOutdoorHydrant_A'` mesh 기반으로 동작 중. 본 변형도 그 규약을 따른다.

### 모델 스케일 주의 (sphere radius 기본값 결정)

루트 `scale [1000, 1000, 1000]` 적용 후 실제 장면 크기는 약 **5.94 × 6.86 × 0.20 단위** (Marker 계열 12개 모두 동일 베이스 판 지오메트리 — 동일 수치). maxDim ≈ 6.86. 보호 영역 구체의 기본 반경은 `maxDim * 1.5 ≈ 10.3` 단위로 설정하여 Marker_OutdoorHydrant 판을 충분히 둘러싸도록 한다 (#62/#63 동일 공식 — `maxDim × 1.5` 그대로 유지). GasDetector(maxDim ≈ 0.7)와 비교하면 동일 sensorHud 패턴이라도 Marker 계열은 root scale 1000 적용 후 절대 크기가 훨씬 크다.
...
**실제 채택**: MeshStateMixin + 커스텀 메서드 `this.emergencyZoneRadius` (신규 Mixin 없음 — #62/#63 답습)

### 결정 근거

1. **#62 Marker_Firealarm + #63 Marker_IndoorHydrant/emergencyZoneRadius 답습** — 동일 변형명·동일 도메인(emergency zone marker)이므로 시그니처 100% 일치. 신규 Mixin 생성 금지 정책에 따라 커스텀 메서드로 완결. (Mixin 승격 임계점 3개 도달은 메인 루프 외부 수동 작업 권장 — 위 섹션 참조)
...
| Mixin / API | 채널 | 책임 |
|-------------|------|------|
| MeshStateMixin | `material.color` (×1 — Marker_OutdoorHydrant 단일 material `Material #45`) | 데이터 상태 색상 (Standard 승계) |
| emergencyZoneRadius emissive | `material.emissive` + `material.emissiveIntensity` (×1 material) | 비상 시 mesh 광휘 펄스 |
| emergencyZoneRadius sphere | 별도 자체 SphereGeometry + MeshBasicMaterial (Marker_OutdoorHydrant material 무관) | 보호 영역 반투명 시각화 + opacity 펄스 |

세 채널이 직교 — 펄스 활성화 시 데이터 상태 색상 보존, sphere는 별도 자원이므로 mesh material 무관, 펄스 정지 시 emissive만 원복하면 된다.

### sphere 디자인 (GasDetector/sensorHud + #62/#63 답습)
...
- 위치: Marker_OutdoorHydrant mesh의 boundingBox 중심에 `mesh.position.copy(markerCenter)` (mount 시 1회 설정 — Marker는 정적이므로 매 프레임 갱신 불요)
...
| color (emergencyColor) | 0xff3030 (적색) | 비상 emissive + sphere 색상 표준 빨강 (옥외 소화전도 화재 비상 도메인이므로 #62/#63과 동일) |
...
### 원본 보관/복원 정책 (#62/#63 답습 — 단일 material 단순화)

`_originals: { savedEmissive: Color, savedIntensity: number }` — 단일 mesh × 단일 material.

- `start()` 또는 `setEmergency({ active: true })` 첫 호출 시점에 mesh의 단일 material(`Material #45`)에 대해 `emissive.clone()`과 `emissiveIntensity`를 보관
- `stop()` / `setEmergency({ active: false })` / `destroy()` 시 보관된 `emissive`를 `material.emissive.copy(saved)`로 복원, `emissiveIntensity`도 원래 값 복원
- material이 emissive 속성이 없는 타입(MeshBasicMaterial 등)이면 silent skip — 단, `Material #45`는 PBR이므로 emissive 지원
...
### 커스텀 네임스페이스 `this.emergencyZoneRadius` (#62/#63과 시그니처 100% 동일)
...
본 변형은 **equipmentStatus를 emergency 트리거로 직접 사용하지 않는다**. equipmentStatus는 Standard 승계로 `meshState.renderData`만 수행하여 색상 채널만 갱신. emergency(active) 토글은 페이지가 별도 토픽(예: `fireAlarmTrigger` 또는 BMS 어댑터)을 받아 `instance.emergencyZoneRadius.setEmergency({...})` 외부 명령형 호출로 위임한다 (#62/#63과 동일 정책 — 화재 비상은 status와 직교한 별도 신호).
...
| radius | `maxDim * 1.5` (mount 시점 boundingBox 기반, ≈ 10.3 단위) | mount 직후 Marker_OutdoorHydrant을 둘러싸는 반투명 구체 즉시 표시 |
...
| autoStart on mount | true | preview/operations 시각 관찰 우선 (#62/#63 동일 정책) |
...
없음. 개별 단위(meshName='MarkerOutdoorHydrant_A' 확정)이므로 `@meshClicked` 같은 동적 식별 이벤트 불필요.
...
| sphere (별도 mesh) | 없음 | Marker_OutdoorHydrant 주변 부유 (보호 영역) |
...
## #62/#63 답습 명세 (시그니처 100% 동일)

| 측면 | #62 Marker_Firealarm | #63 Marker_IndoorHydrant | #64 Marker_OutdoorHydrant (본) |
|------|----------------------|--------------------------|--------------------------------|
| 변형명 | emergencyZoneRadius | emergencyZoneRadius | emergencyZoneRadius |
| 도메인 | 화재 경보기 | 옥내 소화전 | 옥외 소화전 (대칭쌍) |
| meshName | `MarkerFirealarm_A` | `MarkerIndoorHydrant_A` | `MarkerOutdoorHydrant_A` |
| material id | `Material #38` | `Material #41` | `Material #45` |
| root scale | [1000,1000,1000] | [1000,1000,1000] | [1000,1000,1000] (동일) |
| maxDim | ≈ 6.86 | ≈ 6.86 | ≈ 6.86 (동일 베이스 판) |
| sphere radius factor | 1.5 (≈10.3) | 1.5 (≈10.3) | 1.5 (≈10.3) (동일) |
| color | 0xff3030 | 0xff3030 | 0xff3030 (동일) |
| period | 900 ms | 900 ms | 900 ms (동일) |
| idleOpacity | 0.18 | 0.18 | 0.18 (동일) |
| maxOpacity | 0.55 | 0.55 | 0.55 (동일) |
| maxEmissive | 1.5 | 1.5 | 1.5 (동일) |
| API | setEmergency/setRadius/setColor/setPulseOptions/show/hide/start/stop/destroy | **완전 동일** | **완전 동일** |
| 자동 start | ✓ (mount 직후) | ✓ (mount 직후) | ✓ (mount 직후, 동일) |
| 시드 active | false | false | false (동일) |
| material 경로 | 단일 (Array.isArray 안전 가드) | 단일 (동일) | 단일 (동일) |
...
```javascript
// loaded.js
this.markerInst = wemb.getDesignComponent('Marker_OutdoorHydrant');
...
this.markerInst.emergencyZoneRadius.setRadius(20.0);    // 옥외 소화전별 호스 도달 거리 조정 (옥외는 옥내보다 일반적으로 큼)
...
- `models/Marker_OutdoorHydrant/01_default/Marker_OutdoorHydrant.gltf`의 단일 메시 이름은 `'MarkerOutdoorHydrant_A'`로 확정 (폴더명 `Marker_OutdoorHydrant`과 다름 — 구분자 위치 + 접미 불일치). emergencyZoneRadius는 `getObjectByName('MarkerOutdoorHydrant_A')`로 boundingBox 산출 + emissive 변조 대상 mesh를 직접 조회.
- Marker_OutdoorHydrant은 root scale [1000,1000,1000] 적용 후 실제 ~5.94 × 6.86 × 0.20 단위 (Marker 계열 12개 모두 동일 베이스 판). maxDim ≈ 6.86. sphere 기본 반경 maxDim × 1.5 ≈ 10.3 단위로 Marker 판을 충분히 둘러싸도록 설정.
- material은 단일 객체(`Material #45`, PBR) — emissive 속성 지원. `roughness 0.450053632`는 emissive 채널과 무관하므로 펄스 동작에 영향 없음.
...
## Phase 1.5 자율검증 결과 (기준 = Marker_IndoorHydrant/Advanced/emergencyZoneRadius #63)
...
| 5 | 커스텀 메서드 시그니처 일관성 | OK — `setEmergency/setRadius/setColor/setPulseOptions/show/hide/start/stop/destroy` (#62/#63과 100% 동일) |
...
| 8 | manifest + Marker_OutdoorHydrant/CLAUDE.md 등록 | 본 사이클에서 처리 (ADVANCED_QUEUE는 메인 루프 담당) |

---

## 발견한 문제 / Mixin 승격 후보

- **Mixin 승격 임계점 3개 도달 (확정)**: #62 Marker_Firealarm + #63 Marker_IndoorHydrant + #64 Marker_OutdoorHydrant 세 사이클이 동일 시그니처 100% 일치를 입증했다. ADVANCED_QUEUE.md "커스텀 메서드 vs 신규 Mixin 판단 규칙"의 (1) 기법 동일성 + (2) API 호환성을 만족하므로 강력한 승격 신호이며, **2번째 등록 시점 검토 규칙을 이미 초과**한 상태이다.
- **승격 후보 이름**: `EmergencyZoneRadiusMixin`(가칭, 통합) 또는 분리 안 (`RadiusSphereMixin` + `MeshEmissivePulseMixin`)
- **승격 시 분리 권장**: GasDetector/sensorHud의 sphere 부분 + LeakDetector/leakAlarmPulse의 emissive 펄스 부분이 직교하므로:
  - (a) `RadiusSphereMixin`(가칭) — 자체 sphere 생성/dispose + setRadius/setColor + 동적 opacity 변조 옵션 (재사용성 4개 — 본 시리즈 3 + GasDetector/sensorHud)
  - (b) `MeshEmissivePulseMixin`(가칭) — emissive 채널 sine 변조 + 원본 보관/복원 (재사용성 4개 — 본 시리즈 3 + LeakDetector/leakAlarmPulse)
  - 컴포넌트 register에서 두 Mixin을 함께 적용 + 트리거 통합은 컴포넌트 자체 메서드(`setEmergency`)
- **메인 루프 외부 수동 승격 권장**: 본 사이클에서는 메인 루프 신규 Mixin 금지 정책으로 커스텀 메서드 유지하되, 본 사이클(#64) 완료 후 메인 루프 외부에서 사용자 수동 작업으로 `create-mixin-spec` → `implement-mixin` 호출 권장.
