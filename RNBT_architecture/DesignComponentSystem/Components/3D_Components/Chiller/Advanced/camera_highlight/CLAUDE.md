# Chiller — Advanced/camera_highlight

## 기능 정의

1. **상태 색상 표시** — equipmentStatus 토픽으로 수신한 데이터에 따라 'chiller' Mesh 색상 변경
2. **카메라 포커스** — 클릭 시 카메라를 chiller로 이동
...
없음. 페이지 `@chillerClicked` 핸들러가 `cameraFocus.focusOn({ meshName: 'chiller' })` + `meshHighlight.clearAll() → meshHighlight.highlight('chiller')`를 호출한다.
