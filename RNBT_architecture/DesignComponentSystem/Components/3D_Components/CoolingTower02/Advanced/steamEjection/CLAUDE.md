# CoolingTower02 — Advanced/steamEjection

## 기능 정의

1. **상태 색상 표시 (Standard 승계)** — equipmentStatus 토픽으로 수신한 데이터에 따라 'root' Group 노드(자식 mesh CT31__1__CT31 본체, CT31_ladder_A 사다리 포함)의 material.color 변경. Standard와 동일한 colorMap 사용.
2. **상승 수증기 파티클 시각화** — CoolingTower02 mesh 상단(local +Y 오프셋)에 분출구를 두고 `THREE.Points` + `BufferGeometry` 기반 파티클 풀(최대 N=200)을 생성. 매 프레임 RAF에서 alive 입자의 position(Y 상승)/age 갱신. age >= lifetime이면 dead 표시 후 emissionRate 비례로 새 spawn 슬롯에 재활용. 풀 크기는 register 시점에 1회 할당된 BufferGeometry attribute에 in-place mutation만 수행 (재할당 없음).
...
4. **RAF idle 일시정지** — emissionRate = 0이면서 alive 파티클 0개일 때 RAF 자체를 stop(비용 0). emissionRate이 다시 > 0이 되거나 아직 alive 파티클이 남아있으면 RAF 재시작. (#35 답습)
...
## CoolingTower02 mesh 구조

| 항목 | 값 |
|------|-----|
| GLTF 경로 | `models/CoolingTower02/01_default/CoolingTower02.gltf` |
| 분출 기준 mesh 이름 | `root` (GLTF 최상위 Group 노드, scale 1000x — 자식 CT31__1__CT31 본체, CT31_ladder_A 사다리, Standard와 동일) |
| 결정 | **단일 group** (Standard와 동일) |
| 본체 world Y 범위 | 약 -4.7 ~ +38.2 (scale 1000 적용 후) |

> CoolingTower02는 GLTF 최상위 Group 이름이 `'root'`이고 scale 1000이 적용되어 있어, world space 본체 높이가 ~38 units. CoolingTower(#35, scale 미적용, mesh명 'CoolingTower')와는 mesh 이름과 spatial scale이 모두 다르다. 동일 군이지만 명백히 다른 모델 형상이라 spatial 기본값을 일괄 스케일업.

---

## #35 답습 명시 (필수)

본 변형은 직전 사이클 #35 `CoolingTower/Advanced/steamEjection`을 **그대로 답습**한다. 답습 모범의 답습 답습 (#35 자체가 #29~#33 패턴 답습).

| 항목 | #35 (CoolingTower) | 본 변형 (CoolingTower02) | 차이점 |
|------|-------------------|--------------------------|--------|
| MESH_NAME | `'CoolingTower'` | `'root'` | GLTF Group 이름 차이 |
| GLTF root scale | 1.0 (미적용) | 1000.0 | spatial 기본값 일괄 스케일업 필요 |
| `riseSpeed` 기본값 | 1.5 | 30.0 | × 20 |
| `spread` 기본값 | 0.3 | 6.0 | × 20 |
| `pointSize` 기본값 | 0.15 | 3.0 | × 20 |
| `emissionOriginOffset.y` 기본값 | 2.0 | 40.0 | 본체 top ~38 보다 약간 위 |
| `lifetime` 기본값 | 2.5 | 2.5 | 동일 (시간 차원) |
| `temperatureMap` 기본 | `{0, 60, 0, 80}` | 동일 | 동일 |
| `opacity` 기본 | 0.7 | 0.7 | 동일 |
| `color` 기본 | 0xffffff | 0xffffff | 동일 |
| 풀 크기 N | 200 | 200 | 동일 |
| RAF 패턴 | self-null + idle 일시정지 | 동일 | 동일 |
| 채널 분리 | MeshState ↔ Points | 동일 | 동일 |
| 자동 enable on mount | true | true | 동일 |
| 자동 데모 setTemperature | preview에서 50 | preview에서 50 | 동일 |
| API 시그니처 | 13개 메서드 | 13개 메서드 | 100% 동일 |

API 시그니처는 100% 동일하므로 향후 Mixin 승격 시 그대로 흡수 가능.

답습 모범 (체인):

| 답습 | 항목 |
|------|------|
| `CoolingTower/Advanced/steamEjection` (#35) | 이 변형의 1차 답습 모범 — Points 풀 + RAF + 온도 매핑 + 자동 enable + idle 일시정지 |
...
**큐 설명**: "상부 수증기 파티클 (MeshState+커스텀 메서드) — 직전 사이클 #35 답습"

**실제 채택**: MeshStateMixin + 커스텀 메서드 `this.steamEjection` (신규 Mixin **없음**)

### Mixin 승격 메모 (필수)

> **SteamParticleMixin / RisingParticleMixin 승격 후보 — #35 + 본 변형 = 2개 임계점 도달.**
> **#37 CoolingTower03/steamEjection 완료 시 3개로 임계점 명백히 초과. 본 사이클은 신규 Mixin 금지 정책으로 커스텀 유지. setEmissionRate/setTemperature/setTemperatureMap/setRiseSpeed/setLifetime/setSpread/setColor/setOpacity/setEmissionOriginOffset/start/stop/enable/disable/destroy 시그니처 그대로 흡수 가능.**

큐 #35(CoolingTower) + #36(본 변형 CoolingTower02) = 2개 임계점 도달. #37 CoolingTower03/steamEjection이 동일 기법으로 큐 대기 — 다음 사이클 완료 시 3개 임계점을 명백히 초과한다. 메인 루프 외부에서 사용자가 #37 완료 후 `create-mixin-spec` → `implement-mixin` 호출로 `SteamParticleMixin`(또는 더 일반적인 `RisingParticleMixin`) 승격을 강력 권장.

- **승격 후보 이름**: `SteamParticleMixin` 또는 더 일반적인 `RisingParticleMixin`
- **흡수할 메서드 (15개)**: `setEmissionRate/setTemperature/setTemperatureMap/setRiseSpeed/setLifetime/setSpread/setColor/setOpacity/setEmissionOriginOffset/start/stop/isRunning/enable/disable/isEnabled/destroy`
- **모델별 차이**: `MESH_NAME` (각 컴포넌트 고정값) + spatial 기본값 (모델 scale 차이) → Mixin 옵션으로 노출
...
- `setEmissionOriginOffset({x, y, z})` 외부 조정 (기본: `{x:0, y:40.0, z:0}` — CoolingTower02 본체 top ~38 보다 약간 위)
...
new THREE.PointsMaterial({
    size:            3.0,           // CoolingTower02 scale 보정
    transparent:     true,
    opacity:         0.7,
...
> 특히 본 모델은 GLTF root에 scale 1000 적용되어 있어 fallback 사용 시 Points가 1000배 확대된다. 반드시 scene 직속 attach가 필요.
...
| riseSpeed | 30.0 (units/s) | × 20 | scale 1000 모델에서 실제 상승 관찰 가능 |
| lifetime | 2.5 (s) | 동일 | 30 × 2.5 = 75 단위 상승 후 소멸 |
| spread | 6.0 (m) | × 20 | 본체 폭 (~38) 비례 |
| color | 0xffffff (흰색 안개) | 동일 | 어두운 배경에서 명확히 대비 |
| opacity | 0.7 | 동일 | 반투명 안개 |
| size (PointsMaterial) | 3.0 | × 20 | scale 1000 단위에서 가독성 |
| emissionOriginOffset | `{x:0, y:40.0, z:0}` | × 20 | 본체 top ~38 보다 약간 위 |
...
```javascript
// loaded.js
this.ctInst = wemb.getDesignComponent('CoolingTower02');
...
- `models/CoolingTower02/01_default/CoolingTower02.gltf`의 분출 기준 노드는 `'root'` Group(자식 CT31__1__CT31 본체, CT31_ladder_A 사다리). Standard와 동일.
- GLTF root에 `scale: [1000, 1000, 1000]` 적용 — body world Y 범위 ~ -4.7 ~ +38.2.
...
| # | 항목 | 결과 |
|---|------|------|
| 1 | register.js 평탄 (IIFE 금지) | OK — top-level 평탄 (#35 답습) |
...
| 5 | 커스텀 메서드 시그니처 일관성 | OK — `setXxx/start/stop/isRunning/enable/disable/isEnabled/destroy` (#35와 100% 동일) |
| 6 | UI ↔ API 인자 축 일치 | OK — preview temperature slider °C ↔ `setTemperature`, spread m ↔ `setSpread`, riseSpeed units/s ↔ `setRiseSpeed`, lifetime s ↔ `setLifetime`, opacity 0~1 ↔ `setOpacity`, emission origin Y slider ↔ `setEmissionOriginOffset({y})` (slider 범위만 모델 scale에 맞춰 조정) |
| 7 | 기본값 시각 관찰 | OK — preview 로드 직후 자동 enable + 자동 setTemperature(50) → emissionRate ~67/s + offsetY=40 (본체 top ~38) → 흰색 점들이 CoolingTower02 본체 상부에서 상승하는 것 명확. preview 카메라는 box.getSize 기반 자동 거리 계산으로 본체 + 분출 영역 모두 시야 내. |
...
- **Mixin 승격 임계점 도달 — #35 + 본 변형 = 2개**: #37 CoolingTower03/steamEjection이 동일 기법으로 큐 대기 — 다음 사이클 완료 시 **3개 임계점 명백히 초과**. 메인 루프 외부에서 사용자가 `create-mixin-spec` → `implement-mixin` 호출로 `SteamParticleMixin`(또는 더 일반적인 `RisingParticleMixin`) 승격을 강력 권장. API 시그니처 15개 그대로 흡수 가능.
- **모델 spatial scale 차이**: CoolingTower02는 GLTF root에 scale 1000 적용. spatial 기본값(riseSpeed/spread/pointSize/offsetY)을 ×20 일괄 스케일업. lifetime/temperatureMap/opacity/color는 scale 무관이라 #35 그대로. 이 정책은 향후 Mixin 승격 시 모델 정보 기반 자동 스케일 또는 옵션화로 흡수.
- **scene parent 강제 정책 (강화)**: GLTF root scale 1000이 Points에 전파되지 않도록 반드시 `_steamEjectionParent` 또는 `wemb.threeElements.scene`에 add. appendElement fallback은 절대 회피해야 (1000배 확대됨). 안전 가드만.
- **풀 in-place mutation 정책 (#35 동일)**: register 시점에 BufferGeometry attribute(position/age/lifetime/active)를 1회 할당하고 매 프레임 in-place mutation만. dynamic add/remove는 절대 하지 않는다 (성능 + GC churn 회피).
- **RAF idle 일시정지 정책 (#35 동일)**: `emissionRate === 0 && aliveCount === 0`일 때 RAF stop. ensureLoop 패턴(alarmPulse 답습)으로 emissionRate 변경 또는 alive 입자 잔존 시 RAF 재시작.
- **RAF 메모리 누수 방지 (#35 동일)**: destroy에서 반드시 cancelAnimationFrame + null 처리. disable은 visible=false만 (자원 유지). enable은 RAF 재진입 안전(중복 시작 방지).
