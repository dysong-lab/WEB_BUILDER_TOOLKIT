...
this._recomputeAncestors = function (id) {
  ...
  while (parentId) {
    ...
    const allSelected =
      ...
      childStates.every((child) => child.selected && !child.indeterminate);
    const allUnselected = childStates.every(
...
this._renderTree = function ({ response } = {}) {
  ...
  const tree = this.appendElement.querySelector(
    this.treeRender.cssSelectors.container,
  );
