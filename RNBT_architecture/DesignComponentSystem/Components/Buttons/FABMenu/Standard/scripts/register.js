...
this.renderFabMenu = function(data = {}) {
    ...
    const trigger = this.appendElement.querySelector(this.listRender.cssSelectors.trigger);
    const triggerIcon = this.appendElement.querySelector(this.listRender.cssSelectors.triggerIcon);
    const triggerLabel = this.appendElement.querySelector(this.listRender.cssSelectors.triggerLabel);
    if (!root || !trigger || !triggerIcon || !triggerLabel) return;

    const nextIcon = data?.triggerIcon === null || data?.triggerIcon === undefined ? 'add' : String(data.triggerIcon);
    const nextLabel = data?.triggerLabel === null || data?.triggerLabel === undefined ? 'Create' : String(data.triggerLabel);

    triggerIcon.textContent = nextIcon;
    triggerLabel.textContent = nextLabel;
    trigger.setAttribute('aria-label', nextLabel);
    root.dataset.open = 'false';
    this.listRender.renderData({ response: Array.isArray(data?.items) ? data.items : [] });
};
