...
this._clampPosition = function (nextX, nextY) {
  ...
  const parentRect = this._fabEl.parentElement.getBoundingClientRect();
  const maxX = Math.max(0, parentRect.width - this._fabEl.offsetWidth);
  const maxY = Math.max(0, parentRect.height - this._fabEl.offsetHeight);
