...
this.renderExtendedFabInfo = function(data = {}) {
    ...
    const nextLabel = data?.label === null || data?.label === undefined ? '' : String(data.label);
    const nextAria = data?.ariaLabel === null || data?.ariaLabel === undefined ? nextLabel : String(data.ariaLabel);
    const nextSize = data?.size === 'small' || data?.size === 'large' ? data.size : 'medium';
