...
전체 2D 범주(3D_Components, Charts 제외):

```
depth 1: AppBars, Badges, Cards, Checkbox, Dialogs, Divider, Lists, Menus,
         Radio, Search, Snackbar, Switch, Tables, Tabs, TextFields,
         Toolbars, Tooltips, Trees
depth 2: Buttons/{ButtonGroups,Buttons,ExtendedFABs,FAB,FABMenu,IconButtons,SegmentedButtons,SplitButtons}
...
알파벳 순서 전체 목록에서 `register.js`가 없는 첫 번째 항목 = 다음 대상.
...
- 01_refined: Refined Technical (다크 퍼플 / Pretendard / 그라디언트)
- 02_material: Material Elevated (라이트 블루 / Roboto / shadow)
- 03_editorial: Minimal Editorial (웜 그레이 / Georgia 세리프 / 미니멀)
- 04_operational: Dark Operational (다크 시안 / JetBrains Mono / 컴팩트)

각 페르소나의 색상/폰트/스타일은 직전 참고 사례의 동일 페르소나 CSS를 기반으로 한다.
...
- 수동 버전: `/.claude/commands/produce-standard-loop.md` (승인 기반, 기존 유지)
