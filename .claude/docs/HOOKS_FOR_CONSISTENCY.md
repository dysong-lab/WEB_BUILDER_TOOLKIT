...
```
Hook 스크립트는 셸에서 실행된다 (LLM 토큰을 소모하지 않음).

[위반 없음] 스크립트 실행 → exit 0 → 추가 토큰 비용 없음
[위반 발견] 스크립트 실행 → exit 2 + stderr → 피드백이 컨텍스트에 추가 → 재작성
```

| 상황 | 추가 토큰 비용 |
|------|--------------|
| 위반 없음 | **0** (셸 스크립트 실행 시간만) |
| 위반 발견 | stderr 피드백 + 재작성 (위반을 잡는 것이므로 정당한 비용) |

Hook 자체의 오버헤드는 미미하다. 다만 **문서를 읽고 처음부터 올바르게 작성하면
Hook이 개입할 일 자체가 없으므로**, "작업 전 필수 확인"이 가장 효율적인 1순위 메커니즘이다.
Hook은 이를 대체하는 것이 아니라, **실제 반복 실수가 확인된 소수 패턴에만** 적용하는 보험이다.

### 6.2 Hook의 검증 범위는 제한적이다

| 검증 가능 | 검증 불가 |
|-----------|----------|
| 정규식으로 잡히는 단순 패턴 | 아키텍처 설계의 올바름 |
| `<link>` 태그 유무, 인자 개수 | 컴포넌트 구조가 Figma와 일치하는가 |
| 폐기된 도구명, 구조분해 패턴 | 코딩 스타일, fx.js 파이프라인 관례 |

대부분의 규칙은 정규식으로 표현할 수 없으며, Hook으로 강제할 수 없다.
Hook이 잡을 수 있는 4개 패턴은 전체 규칙의 극히 일부이다.
...
대응:
  ...
  - Phase 1에서 피드백 모드로 운영하며 오탐 패턴 수집
  - 수집 후 정규식 정밀화
```

### 6.4 문서가 핵심이고 Hook은 보조이다

```
규칙 준수의 주된 메커니즘:
  1순위: SKILL "작업 전 필수 확인" → Claude가 참조 문서를 읽고 올바르게 작성
  2순위: Hook → 반복 실수 패턴에 대한 안전망 (선택적 적용)

Hook은 문서를 대체하지 않는다.
  - 문서: 왜 이 규칙이 존재하는지, 정확한 정의, 올바른 패턴 설명
  - Hook: 알려진 소수의 실수 패턴을 자동 검출
```
...
│    규칙 A ──→ 참조 문서 1곳 (단일 출처)                      │
│    규칙 B ──→ 참조 문서 1곳 (단일 출처)                      │
│    SKILL  ──→ "작업 전 필수 확인" + 참조 링크 + 고유 내용    │
│                                                             │
│    ✅ 불일치 위험 제거                                       │
│    ✅ "작업 전 필수 확인"이 규칙 준수의 주된 메커니즘         │
│                                                             │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  선택적 적용: Hook (반복 실수 패턴에만)                       │
│                                                             │
│    4개 패턴 ──→ PostToolUse 스크립트로 자동 검출              │
│                                                             │
│    ⚠️ 전면 도입이 아닌 선택적 안전망                         │
│    ⚠️ 토큰 비용 증가 가능 — 실제 반복 실수 확인 후 도입      │
│                                                             │
└─────────────────────────────────────────────────────────────┘

핵심: "작업 전 필수 확인 → 문서 읽기 → 올바른 작성"이 1순위.
      Hook은 반복 실수가 확인된 소수 패턴에 대한 보조 안전망.
```
