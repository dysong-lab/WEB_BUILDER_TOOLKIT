...
이 역할 경계가 무너지면, 형식은 완벽해도 설계가 무너진 것이다. 아래는 실제로 발생한 위반 사례다.
...
이 계약들은 Hook으로 자동 검출할 수 없다. **리뷰 시 가장 먼저 확인해야 할 항목**이다.

| 계약 | 위반하면 | 자동화 |
|------|---------|--------|
| HTML은 데이터를 모른다 (런타임 데이터 하드코딩 금지) | Mixin 인터페이스가 장식품이 됨 | 리뷰 |
| Mixin은 HTML 구조를 모른다 (선택자 계약으로만 접근) | 디자인 변형이 불가능해짐 | 리뷰 |
| register.js는 조립만 한다 (렌더링 로직, fetch 금지) | 컴포넌트가 껍데기가 아니게 됨 | 리뷰 |
| Mixin은 다른 Mixin을 모른다 (조립 코드가 조합을 주도) | Mixin 간 의존성 발생 | 리뷰 |
| 페이지는 렌더링하지 않는다 (오케스트레이션만) | 역할 경계 붕괴 | 리뷰 |
