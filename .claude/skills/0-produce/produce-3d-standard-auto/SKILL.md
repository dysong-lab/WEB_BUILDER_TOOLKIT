...
description: 3D Standard(01 status) 컴포넌트를 서브에이전트 기반으로 한 장비씩 독립 컨텍스트에서 완전 자동 생산합니다. 메인은 Phase 0(PRODUCTION_QUEUE.md 파악) → Agent 호출 → Phase 2(커밋+큐 업데이트) → 반복.
---
...
DesignComponentSystem/Components/3D_Components 아래의 3D 컴포넌트를 PRODUCTION_QUEUE.md 순서대로 **Standard(01 status)** 생산한다.
...
[메인 루프]
    │
    ├─ Phase 0: PRODUCTION_QUEUE.md에서 다음 대상 파악
    ├─ Phase 1: Agent(subagent_type=general-purpose) 호출 → 단일 장비 Standard 생산 위임
    ├─ Phase 2: 결과 확인 + 커밋 + PRODUCTION_QUEUE.md 업데이트
...
2. "기존(완료)" 컬럼에 `status`가 포함되지 않고, 상태가 "대기"인 첫 번째 항목 = 다음 대상
   - Phase A(개별 장비)와 Phase B(컨테이너) 모두 순서대로 탐색
...
3. Standard 폴더 존재 여부 확인:
   ...
   ```
   이미 존재하면 스킵하고 다음 대상으로 이동.

4. 유형 결정:
   - "개별" → `create-3d-component` 사용
   - "컨테이너" → `create-3d-container-component` 사용
...
**프롬프트 템플릿** (매 사이클 `{장비명}`, `{유형}`, `{개발스킬}`만 교체):

```
대상: 3D 컴포넌트 `3D_Components/{장비명}/Standard`를 처음부터 끝까지 생산한다.
유형: {개별 또는 컨테이너}
...
8. `Components/3D_Components/{장비명}/CLAUDE.md` — 장비 정의 (이미 존재하면)
...
- 컨테이너 참고: `Components/3D_Components/gltf_container/` — 대표 컨테이너 컴포넌트

## 산출물 (모두 자동으로 작성)

1. `{장비명}/CLAUDE.md` — 장비 정의 (없으면 생성, 있으면 Standard 섹션 추가)
2. `{장비명}/Standard/CLAUDE.md` — Standard 변형 구현 명세
3. `{장비명}/Standard/scripts/register.js` — MeshState Mixin 적용 + 구독
4. `{장비명}/Standard/scripts/beforeDestroy.js` — 정리 코드
5. `{장비명}/Standard/preview/01_default.html` — 독립 실행 프리뷰 (모델 있으면 로드, 없으면 placeholder)
6. `DesignComponentSystem/manifest.json` — `{장비명}`의 `sets` 배열에 Standard 엔트리 추가
...
- 주요 구독 토픽
- 이벤트 매핑 (있으면)
- 모델 상태 (준비/미준비 — [MODEL_READY] placeholder 사용 여부)
- 특이 결정사항
- 발견한 문제/의문점 (있으면)
```
...
### Phase 2. 결과 확인 + 커밋 + 큐 업데이트

Agent 반환 후:

1. 생성된 파일 존재 확인:
   ```bash
   ls RNBT_architecture/DesignComponentSystem/Components/3D_Components/{장비명}/Standard/{scripts,preview}/
   ls RNBT_architecture/DesignComponentSystem/Components/3D_Components/{장비명}/Standard/scripts/register.js
   ls RNBT_architecture/DesignComponentSystem/Components/3D_Components/{장비명}/Standard/scripts/beforeDestroy.js
   ```
...
3. 커밋:
   ```
   feat: 3D_Components/{장비명}/Standard 컴포넌트 자동 생산 — {Agent 요약 첫 줄}
   ```

4. PRODUCTION_QUEUE.md 업데이트:
   - 해당 장비의 "기존(완료)" 컬럼에 `status` 추가
   - "추가 대상"에 Advanced가 남아 있으면 상태는 "대기" 유지
   - 남은 항목이 없으면 "완료"로 변경
...
- PRODUCTION_QUEUE: `/RNBT_architecture/DesignComponentSystem/Components/3D_Components/PRODUCTION_QUEUE.md`
