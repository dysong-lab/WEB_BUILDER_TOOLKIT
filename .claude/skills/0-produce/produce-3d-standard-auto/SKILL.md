...
5. `{컴포넌트경로}/Standard/preview/01_default.html` — 독립 실행 프리뷰 (모델 있으면 로드, 없으면 placeholder). **`<script>` 내부 코드는 [`_shared/preview-area-labeling.md`](/.claude/skills/0-produce/_shared/preview-area-labeling.md)의 4종 라벨(`[PREVIEW 인프라]` / `[PAGE]` / `[COMPONENT register.js 본문]` / `[PREVIEW 전용]`)로 영역 분리 필수**
