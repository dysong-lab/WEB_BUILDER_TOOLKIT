...
> **일괄 모드 주의**: 일괄 모드는 "여러 장비에 공통 적용 가능한 패턴"(= 기존 프리셋/Mixin) 쪽으로 사고가 밀린다. 장비별 고유 인터랙션(센서 감지 반경, CCTV 시야각 콘, 유체 흐름, 동적 RPM, 3D HUD, 열화상 분포 등)은 공통 패턴이 아니므로 레이더에서 밀릴 위험이 있다. 일괄 모드 실행 후 각 도메인 대표 1~2개를 골라 단건 모드로 2차 탐색을 돌리는 **2단계 전략**을 권장한다.
...
- **이 단계에서 Mixin 카탈로그/프리셋 목록을 읽거나 참조하지 않는다.** 아래 ②의 수단 목록을 미리 보는 것도 금지.
- 위반 경보: "어떤 Mixin을 쓸까", "프리셋 중 뭐가 맞을까"로 사고가 흐르면 즉시 멈추고 "이 장비에 어떤 기능이 필요한가"로 리셋.
- 예시 탐색축: 운영자가 보는 값(수치/상태) / 조작하는 행위(시작·정지·제어) / 알고 싶은 공간 정보(위치·범위·방향) / 시간에 따른 변화(애니메이션·추세) / 이상 시 필요한 강조.
...
1. **상단**: 기능 발굴 원칙 + 장비 도메인 특성 + 운영 인터랙션 예시. 이 블록 내에 **"Mixin 존재 여부 고려 금지"** 문구를 명시.
2. **중단**: 사고 순서(① 기능 발굴 → ② 구현 수단 매핑).
3. **하단**: 참조용 Mixin/프리셋 목록. 반드시 "**② 단계에서만 참조**"라는 주석과 함께.

**위임 프롬프트 구성 요소** (위 배치 순서로 작성):
- 대상 컴포넌트 목록(개별 + 컨테이너)과 각 컴포넌트의 Standard CLAUDE.md 내용
- 제외 목록 (Step 1에서 수집한 기존 변형)
- 컴포넌트 특성에서 추론 가능한 기능 예시 (예: Chiller는 가동/정지 애니메이션, Panel은 전력 계측 데이터 표시, 컨테이너는 단면 분석/구역 강조, 센서는 실시간 측정값 HUD / 감지 반경 시각화, CCTV는 시야각 콘)
...
> Mixin 카탈로그가 미성숙한 단계에서 수백 종 장비의 모든 Advanced 기능이 기존 N개 Mixin으로 커버된다면, 이는 성과가 아니라 **탐색 실패 신호**다. "100% 커버"를 긍정 신호로 읽지 않는다.

경보 발생 시 다음 중 하나를 반드시 수행:

1. **2차 패스 재탐색**: Explore에 "기존 Mixin으로 표현 불가한 기능만 찾아달라"는 별도 프롬프트로 재위임. 센서 감지 반경, CCTV 시야각 콘, 유체 흐름, 동적 RPM 제어, 3D 공간 HUD, 열화상 표면 그라데이션 등 구체 예시 포함.
