...
Standard 폴더가 없는 장비는 Advanced 대상에서 제외하고 `produce-3d-standard-loop` 선행을 안내한다. Standard 없이 Advanced를 생산하면 MeshState 기반 분리 정당성을 검증할 수 없기 때문이다.
...
2. 상태가 "대기"인 첫 번째 항목 = 다음 대상
   - 예: `Panel | 개별 | highlight | MeshState + MeshHighlight | 대기` → 대상: Panel/Advanced/highlight

3. Standard 선행 필터: 해당 장비의 Standard 폴더 존재 여부 확인
   ```bash
...
4. Advanced 폴더 존재 여부 확인:
   ```bash
   ls RNBT_architecture/DesignComponentSystem/Components/3D_Components/{장비명}/Advanced/{변형이름}/ 2>/dev/null
   ```

5. **사용자에게 보고**: "다음 대상: {장비명}/Advanced/{변형이름} — {설명}"
...
```
{장비명}/Advanced/{변형이름} 생산 완료.
다음 대상: {다음 장비명}/Advanced/{다음 변형}
