...
매 사이클 시작 시 실행한다. 범주 목록을 하드코딩하지 않고 **폴더 구조를 동적 감지**한다. 범주 폴더 안에 서브범주(Standard/Advanced 이외의 하위 디렉토리)가 있으면 depth 2, 없으면 depth 1로 자동 판별. `3D_Components`, `Charts`는 제외.
...
2. 각 후보에 대해 Standard 완료 여부 확인 (`register.js`가 없는 첫 번째 항목 = 다음 대상):
   ```bash
