...
**DOM 패턴**으로 Mixin을 선택한다.

| DOM 패턴 | Mixin | 네임스페이스 |
|----------|-------|-------------|
| 고정 DOM에 외부 데이터/상태 매핑 | FieldRenderMixin | `this.fieldRender` |
| 배열 → template 반복 생성 | ListRenderMixin | `this.listRender` |
| 팝업/오버레이 (Shadow DOM) | ShadowPopupMixin | `this.shadowPopup` |
| 재귀 배열 → 트리 구조 | TreeRenderMixin | `this.treeRender` |
| 차트 | EChartsMixin | `this.echart` |
| 테이블 | TabulatorMixin | `this.tabulator` |
| **Mixin 불필요** | 순수 CSS + 자체 메서드 | — |

**Mixin 불필요 케이스:** 외부 데이터를 받아 DOM에 반영할 필요 없이, 순수 CSS와 이벤트 핸들링만으로 동작하는 컴포넌트 (예: Buttons, Switch, Divider).

> ListRenderMixin에 `itemKey` 옵션을 제공하면 `updateItemState`/`getItemState`로 개별 항목 상태 관리가 가능하다. 그 외 Mixin(HeatmapJs, MeshState, CameraFocus)은 해당 Mixin의 .md 문서를 참고한다. 전체 목록: [Mixins/README.md](/RNBT_architecture/DesignComponentSystem/Mixins/README.md)
