...
`preview/*.html`의 `<script>` 내부 코드는 빌더(다른 페이지 개발자)가 컴포넌트 사용법을 학습하는 **가이드** 역할을 한다. 따라서 다음 4종 라벨로 영역을 분리하여 페이지 책임 코드와 컴포넌트 register.js 본문이 한눈에 구분되도록 작성한다. 컨테이너 컴포넌트는 페이지가 외부 DOM 마운트(예: HUD overlay) + `instance.hudRoot` 등 외부 자원 주입까지 담당하는 경우가 많아 본 규약 적용이 특히 중요하다.

| 라벨 | 의미 |
|------|------|
| `[PREVIEW 인프라]` | Three.js scene/camera/light 셋업, GLTFLoader, model.traverse mesh 이름 수집, OrbitControls, originalColors 저장 등 운영과 무관한 보일러플레이트 |
| `[PAGE]` | 페이지 개발자가 운영에서 직접 작성할 코드 (외부 DOM 정적 렌더 시뮬레이션 함수, `instance.hudRoot` / `instance.renderer` 외부 자원 주입, `pageDataMappings` 시뮬레이션) |
