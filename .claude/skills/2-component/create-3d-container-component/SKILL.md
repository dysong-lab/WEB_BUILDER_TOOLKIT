...
각 변형(01, 02, 03)의 `component/register.js`를 읽고 확인:

- ❌ 위반: innerHTML, appendChild, createElement 등 DOM 조작
- ❌ 위반: fetch, XMLHttpRequest, axios 호출
- ❌ 위반: 데이터 가공/변환 로직
- ✅ 정상: applyMixin + subscribe + bindEvents (+ resolveMeshName/showDetail 정의)만 존재

※ resolveMeshName, showDetail 같은 인스턴스 메서드 정의는 "이벤트 응답 인터페이스"로 허용된다.
