...
- 네임스페이스 이름 (섹션 4)
- 메서드/속성 목록 (섹션 4)
- **메서드 입력 포맷** — 각 메서드의 파라미터/타입/필수/기본값/반환 (섹션 5)
- destroy 범위 (섹션 6)
...
- [ ] **각 메서드의 파라미터/반환이 명세서 섹션 5의 "메서드 입력 포맷"과 일치하는가** (타입·필수·기본값·반환 타입 모두)
- [ ] destroy에서 명세서 섹션 6의 모든 항목을 정리하는가
...
2. **명세서의 섹션 5 (메서드 입력 포맷)와 .js 구현이 1:1 대응하는가** — 파라미터 이름·개수·타입·기본값·반환 타입
3. 명세서의 섹션 6 (destroy 범위)와 .js의 destroy 내부가 1:1 대응하는가
4. .md API 문서의 "메서드 입력 포맷" 섹션이 .js 구현과 일치하는가 (타입 오기 없는가)
5. 갱신한 문서들에서 Mixin 수가 전체적으로 일치하는가
