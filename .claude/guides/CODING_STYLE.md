...
**헤더와 데이터 행의 시작점(offset)을 동일하게 맞춰야 컬럼이 정렬됩니다.**

```
❌ 정렬 안 맞음:
헤더:  [18px 빈공간][발생시간][타입][장비][내용]
데이터: [24px 빈공간][시간값 ][EVA][SV001][고온...]
        ↑ 6px 차이로 모든 컬럼이 밀림

✅ 정렬 맞음:
헤더:  [24px 빈공간][발생시간][타입][장비][내용]
데이터: [24px 빈공간][시간값 ][EVA][SV001][고온...]
```
...
```css
.data-row {
    padding-left: 7px;      /* 행 패딩 */
}
.indicator {
    width: 11px;            /* 요소 너비 */
    margin-right: 6px;      /* 요소 마진 */
}
/* 총 시작 간격: 7 + 11 + 6 = 24px */
```
...
**헤더 행에 동일한 간격 적용:**
```css
.header-row .indicator-space {
    width: 24px;            /* 데이터 행과 동일 */
}
```

**핵심:** 헤더에 실제 요소가 없더라도, 데이터 행의 `padding + 요소 width + margin` 합계와 동일한 빈 공간을 확보해야 합니다.
